namespace Algorithm
    ...
    module CSVReader=
        ...
        let read (inputPath:string) = 
            ...
            let readLines:string[] = IO.File.ReadAllLines(inputPath)
            let prices = new Collections.Generic.List<Tuple<DateTime, decimal, decimal, decimal, decimal,int64>>()
    ...
    //                date <- date.AddHours(12.0);
                let a = (new Tuple<DateTime,decimal,decimal,decimal,decimal,int64>(date, (decimal ((readLines.[i]).Split([|';'|]).[2])), (decimal ((readLines.[i]).Split([|';'|]).[3])), 
