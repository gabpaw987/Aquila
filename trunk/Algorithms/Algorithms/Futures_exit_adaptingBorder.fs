...
//        let mutable ns = new System.Collections.Generic.List<decimal>()
        ...
        let startCalculation (prices:System.Collections.Generic.List<System.Tuple<System.DateTime, decimal, decimal, decimal, decimal, int64>>, 
            ...
            let rsiN = 18
            let rsiEmaN = 5
            let mutable rsiLong = 80m
            let mutable rsiShort = 20m
            ...
            let mutable rsiLong = parameters.["rsiLong"]
            let rsiExitLong = parameters.["rsiExitLong"]
            let mutable rsiShort = parameters.["rsiShort"]
            ...
            // Adaptive Borders
            let adaptExtrN = int parameters.["adaptExtrN"]
            let adaptBarExtrN = int parameters.["adaptBarExtrN"]
            let adaptN = int parameters.["adaptN"]
            ...
            for i in 0 + signals.Count - recalcN .. prices.Count-1 do
                ...
                if (i < firstI || missingData > 0) (*|| signals.Count - 1 < i*) then
                    ...
                    if adaptN <> 0 && i >= adaptExtrN then
                        ...
                        let mins, maxs = getExtremeValues(adaptExtrN, cPrices.[firstExtrI..if(i-adaptBarExtrN/2 > 0) then i-adaptBarExtrN/2 else 0], borderExtremas.[firstExtrI..if(i-adaptBarExtrN/2 > 0) then i-adaptBarExtrN/2 else 0])
                        
                        if mins.Count + maxs.Count > adaptN then
                            let minD = if mins.Count > 1 then regressionSlope([|for i in mins do yield i|]) else 0m
                            let maxD = if maxs.Count > 1 then regressionSlope([|for i in maxs do yield i|]) else 0m
                            if sign minD <> sign maxD && minD <= adaptSlope * decimal (sign minD) && sign minD = -1 && maxD >= adaptSlope * decimal (sign maxD) && sign maxD = 1 then 
                                rsiShort <- 40m
                                rsiLong <- 60m
                            else if minD >= adaptSlope && maxD >= adaptSlope && sign minD = 1 then 
                                rsiShort <- 20m
                                rsiLong <- 60m
